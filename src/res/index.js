export default ({ thenables, catchables }) => ({
  handleThenables: function (_name, _method, _resObj) {
    ...
    if (found) {
      wasFoundThroughMethod ? thenables[Object.keys(thenables)[index]](_resObj, _name) : thenables[Object.keys(thenables)[index]](_resObj)
    }
    else {
      return _resObj;
    }
  ...
  handleCatchables: function (_name, _method, _errorObj) {
    ...
    if (found) {
      wasFoundThroughMethod ? catchables[Object.keys(catchables)[index]](_errorObj, _name) : catchables[Object.keys(catchables)[index]](_errorObj)
    }
    else {
